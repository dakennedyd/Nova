...
vec2 mytrunc(vec2 x, vec2 num_levels) { return floor(x * num_levels) / num_levels; }
...
vec4 chromatic(int sampleCount, float blur, float falloff)
{
    ...
    // vec2 velocity = direction * blur * pow(length(vec2(lerpedTexCoords.x,0.0) -vec2(0.5,0.0)),
    // falloff);
    float inverseSampleCount = 1.0 / float(sampleCount);

    mat3x2 increments =
        mat3x2(velocity * 1.0 * inverseSampleCount, velocity * 2.0 * inverseSampleCount,
...
vec3 FxaaPixelShader(vec4 uv, sampler2D tex, vec2 rcpFrame)
{
    ...
    vec3 rgbA = (1.0 / 2.0) * (textureLod(tex, uv.xy + dir * (1.0 / 3.0 - 0.5), 0.0).xyz +
                               textureLod(tex, uv.xy + dir * (2.0 / 3.0 - 0.5), 0.0).xyz);
    vec3 rgbB = rgbA * (1.0 / 2.0) +
                (1.0 / 4.0) * (textureLod(tex, uv.xy + dir * (0.0 / 3.0 - 0.5), 0.0).xyz +
...
void main()
{
    ...
    fragment = vec4(FxaaPixelShader(uv, uRenderedImage, rcpFrame), 1.0) * 0.65 +
               texture(uBlurredImage, lerpedTexCoords) * 0.07;
    // fragment = texture(uRenderedImage, lerpedTexCoords)*0.65 + texture(uBlurredImage,
    // lerpedTexCoords)*0.04; fragment = texture(uBlurredImage, lerpedTexCoords); fragment =
    // (texture(uRenderedImage, lerpedTexCoords) + texture(uBlurredImage,
    // lerpedTexCoords))+glitch(0.4); fragment = (texture(uRenderedImage, lerpedTexCoords) +
    // texture(uBlurredImage, lerpedTexCoords))+glitch(0.4)+chromatic(50, 0.25,2.0+sin(uTime));
