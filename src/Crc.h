...
/*
      ...
      for (byte = 0; byte <= 255; byte++) {
        ...
        static uint32_t crc32Table[256] = { // Calculated from polinomial
   0xEDB88320 0x00000000, 0x77073096, 0xee0e612c, 0x990951ba, 0x076dc419,
   ...
   0x2a6f2b94, 0xb40bbe37, 0xc30c8ea1, 0x5a05df1b, 0x2d02ef8d
        ...
        static uint32_t SID(const uint8_t *data, size_t dataSize)
        {
                uint32_t result = 0xFFFFFFFF;
                for (size_t i = 0; i < dataSize; i++) {
                        result = (result >> 8) ^ crc32Table[(result ^ data[i]) &
   0xFF];
                }
                return(result ^ 0xFFFFFFFF);
        }

        static uint32_t SID(const std::string &str)
        {
                return SID(reinterpret_cast<const uint8_t *>(str.c_str()),
   str.length());
        }*/
...
constexpr unsigned crc32Table[] = { A(0) };*/
constexpr uint32_t crc32Table[256] = { // Calculated from polinomial 0xEDB88320
...
{
    return len ? crc32_impl(p + 1, len - 1, (crc >> 8) ^ crc32Table[(crc & 0xFF) ^ *p]) : crc;
}
...
constexpr size_t strlen_c(const char *str) { return *str ? 1 + strlen_c(str + 1) : 0; }

constexpr int SID(const char *str) { return crc32((uint8_t *)str, strlen_c(str)); }
} // namespace Nova
