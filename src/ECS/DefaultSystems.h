...
namespace Nova {
	...
	class VisualSystem final :public System
	{
		...
		void onRegister(Entity* entity) override
		{
			...
			//vc.renderPacketID = GraphicsSystem::getInstance().getRendererBackend().addPacket(RenderPacket{ vc.mesh, vc.material, &entity->getTransformStruct() });
		}
	...
	class LightSystem final :public System
	{
		...
		void onRegister(Entity* entity) override
		{
			...
			if (lc.castsShadow)
			{
				lc.shadowFramebuffer = std::make_shared<FrameBuffer>(
					FrameBuffer::makeShadowMapFrameBuffer());
			}
		}
	...
	class PlayerInputSystem final :public System
	{
		void processEntity(Entity* entity) override
		{
			...
			//entity->rotate(UnitQuat(-mouseVector.y* mouse.getSensitivity(), mouseVector.x* mouse.getSensitivity(), 0.0f));
			//entity->rotate(dir, toRadians(mouseVector2.getMagnitud() * mouse.getSensitivity()));
	...
	class CameraSystem final :public System
	{
		void processEntity(Entity* entity) override
		{
			...
			camera.view = Mat4::makeLookAtMatrix(pos,
					pos - rot * ts.forward,
					rot * ts.up);
