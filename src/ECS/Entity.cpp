...
namespace Nova {
	Entity::Entity(const std::string & name)
		:mName(name), mEntityID(SID(name.c_str()))
	{
	}

	Entity::Entity(const std::string & name, const uint32_t id)
		: mName(name), mEntityID(id)
	{
	}
	...
	void Entity::move(const Vec3 & vector)
	{
		mTransform.translation = mTransform.translation + vector;
	}
