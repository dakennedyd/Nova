...
namespace Nova {
	...
	struct Transform
	{
		Transform() :finalTransform(Mat4::makeIdentityMatrix()), /*transform(Mat4::makeIdentityMatrix()),*/ normalMatrix(finalTransform.calcNormalMatrix()) {};
		Transform(const Mat4 & transform) :finalTransform(transform), /*transform(Mat4::makeIdentityMatrix()),*/ normalMatrix(transform.calcNormalMatrix()){};
	...
	public:
		...
		void addComponent(Params&&... parameters)
		{ 
		...
		{			
			...
			mSubscribedSystems.insert(std::make_pair(std::type_index(typeid(T)), World::GetSystem<T>()));
		};*/
		//void registerEntity();		//notifies the relevant systems based on the entity's components
	...
	private:
		...
		Entity(const std::string &name, const uint32_t id); //this ctor is only so that World doesn't have to generate id twice
		std::string mName;
		uint32_t mEntityID;
		std::unordered_map<std::type_index, IComponent*> mComponents; //only one of each type of component is allowed		
