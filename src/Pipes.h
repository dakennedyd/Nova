...
namespace Nova {
	...
	struct Pipe
	{
		...
		virtual auto operator () () const -> outputType
		{
			return outputType();
		}
	};
	...
	auto operator>> (const Pipe <INPUT, OUTPUT_INPUT> &lhs,
		Pipe <OUTPUT_INPUT, OUTPUT> &rhs) -> Pipe <OUTPUT_INPUT, OUTPUT>&
	{
		rhs = lhs();
		return rhs;
	}
	...
	{
		SortRenderPackets(const std::vector<int>& packets)
			:Pipe<inputType, outputType>(packets)
		{
		}
		auto operator () () const -> outputType
		{
