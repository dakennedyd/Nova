...
namespace Nova {
	...
	void FrameBuffer::attachTexture(const std::shared_ptr<Texture> texture)
	{
		...
		if (texture->getWidth() > maxFBWidth) { error("framebuffer width limit exceeded"); };
		if (texture->getHeight() > maxFBHeight) { error("framebuffer height limit exceeded"); };
		if (texture->getWidth() < 1) { error("Invalid framebuffer width"); };
		if (texture->getHeight() < 1) { error("Invalid framebuffer height"); };
		...
		if (texType == TextureType::DEPTHBUFFER)
		{
			...
			glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
				GL_RENDERBUFFER, texture->getTextureID());
		}
		...
		else if (texType == TextureType::COLOR || texType == TextureType::COLOR_ALPHA|| texType == TextureType::COLOR_ALPHA
			|| texType == TextureType::GBUFFER_NORMAL || texType == TextureType::GBUFFER_POSITION
			...
			|| texType == TextureType::MONOCHROME)
		{
			...
			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + 
				static_cast<int>(mColorTextures.size()) - 1, GL_TEXTURE_2D,
				texture->mTextureID, 0);
		}
		else { LOG_ERROR("Unrecognized texture type for framebuffer attachment") };
	...
	//	{
	//		mColorTextures.emplace_back(std::make_shared<Texture>(width, height, nullptr, TextureType::COLOR,
	//			Filtering::NEAREST));
	...
	//	{
	//		mColorTextures.emplace_back(std::make_shared<Texture>(Texture::makeShadowMap(width, height)));
	...
	//		mColorTextures.emplace_back(std::make_shared<Texture>(1, 1, nullptr, TextureType::COLOR, Filtering::NEAREST));			
	//		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, mColorTextures[1]->mTextureID, 0);
	//	}
	...
	FrameBuffer FrameBuffer::makeDefaultFrameBuffer(const int width, const int height)
	{
		...
		if (!fb.isComplete()) LOG_ERROR("can't create color/depth framebuffer"); //check for framebuffer completeness
		return fb;
	}
	FrameBuffer FrameBuffer::makeGBuffer(const int width, const int height)
	{
		FrameBuffer fb;
		fb.attachTexture(std::make_shared<Texture>(width, height, nullptr, TextureType::GBUFFER_POSITION,
	...
	FrameBuffer FrameBuffer::makePostProcessFrameBuffer(const int width, const int height)
	{
		...
		if (!fb.isComplete()) LOG_ERROR("can't create post process framebuffer"); //check for framebuffer completeness
		return fb;
	}
