...
namespace Nova {
	...
	void GPUProgram::construct(const std::string &vertexShader,
		const std::string &pixelShader, const std::string &filename)
	{
		// ======================== Vertex Shader ========================
		...
		if (GL_TRUE != params) {
			...
			LOG_ERROR("GL vertex shader " + filename + " index " + std::to_string(vertexShaderHandle) + " did not compile");
			error(str);
		}
		...
		if (GL_TRUE != params) {
			...
			std::string str(std::begin(v), std::end(v)-2);			
			LOG_ERROR("GL fragment shader " + filename + " index " + std::to_string(fragmentShaderHandle) + " did not compile");
			error(str);
		}
		...
		/*glBindAttribLocation(handle, 0, "vertex_position");		//attrib location must always be set on the shader
		glBindAttribLocation(handle, 1, "vertex_colour");*/
		...
		if (GL_TRUE != params) {			
			error("could not link shader " + filename + " program GL index " + std::to_string(mProgramID));			
		}
