...
// &properties) 	:mProgram(program), mAmbient(properties.ambient), mDiffuse(properties.diffuse),
//	mSpecular(properties.specular)
//{
//}
// Material::Material(const std::shared_ptr<GPUProgram> &program, const std::shared_ptr<ITexture>
// &diffuse, 	const MaterialProperties &properties) 	:mProgram(program), mAmbient(properties.ambient),
