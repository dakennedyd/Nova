...
namespace Nova {	
	...
	{
		//mHasMipmaps = true;
		if (width < 1) { error("Invalid texture width"); }; mWidth = width;
		if (height < 1) { error("Invalid texture height"); }; mHeight = height;
		int maxTextureSize = 0;
		glGetIntegerv(GL_MAX_TEXTURE_SIZE, &maxTextureSize);
		if(width > maxTextureSize || height > maxTextureSize) { error("Texture dimension too big"); };


		if (textureType == TextureType::DEPTHBUFFER || textureType == TextureType::DEPTHSTENCIL)
		{
			//mBorderColor[0] = 0.0f; mBorderColor[1] = 0.0f; mBorderColor[2] = 0.0f; mBorderColor[3] = 0.0f;
			...
			GLfloat maxAniso = GraphicsSystem::getInstance().getMaxAnisotropy();	//set anisoptropy level
			if (maxAniso >= 4.0f) glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, 4.0f);
		}

		if (textureType == TextureType::GBUFFER_NORMAL || textureType == TextureType::GBUFFER_POSITION
		...
		if (textureType == TextureType::COLOR || textureType == TextureType::COLOR_ALPHA
			...
			if (filtering == Filtering::LINEAR)
			{
				...
				if (mHasMipmaps) glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
				else glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
			}
			else if (filtering == Filtering::NEAREST)
			{
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
				if (mHasMipmaps) glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_NEAREST);
			...
			if (textureType == TextureType::COLOR_HDR) {
				...
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, width, height, 0, GL_RGB, GL_FLOAT, textureData);
			}
	...
	{
		if (textureInfo.bpp == 4) return TextureType::COLOR_ALPHA;
		else if (textureInfo.bpp == 3) {
			if(textureInfo.isHDR) return TextureType::COLOR_HDR;
			else return TextureType::COLOR;
		}
		else if (textureInfo.bpp == 2)return TextureType::DUAL_COLOR;
		else if (textureInfo.bpp == 1)return TextureType::MONOCHROME;
		else {
	...
	Texture::Texture(const TextureInfo& textureInfo)
		:Texture(textureInfo.width,
			...
			//(textureInfo.bpp == 4) ? TextureType::COLOR_ALPHA : (textureInfo.bpp == 3) ? TextureType::COLOR : unrecognizedTextureBuffer())
			//TextureType::COLOR)
	{		
		//bpp == 4 -> GL_RGBA
		//bpp == 3 -> GL_RGB
		//mTextureBuffer = textureInfo.textureBuffer;
		FileSystem::getInstance().freeTexture(textureInfo.textureBuffer[0]); //TODO: texture object should hold a ptr to image data in ram
	}
