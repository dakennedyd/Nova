...
namespace Nova {		
	...
	const float * Vec3::getDataPtr() const
	{
		return mData;
	}
	...
	void Vec3::rotateSelf(const Vec3& rotAxis, const float angle)
	{
		...
		*this = thisVector * cos(angle) +
				(rotAxis^thisVector)*sin(angle) +
	...
	float Vec3::getMagnitud()
	{
		if (mMagnitud == -1.0f)
		{
			mMagnitud = std::sqrt(mData[0] * mData[0]
				+ mData[1] * mData[1]
				+ mData[2] * mData[2]);
		}			
		return mMagnitud;
	...
	float Vec3::getDistance(const Vec3 & v)
	{
		return (*this - v).getMagnitud();
	}
	...
	Vec4::Vec4(const Vec3 & vec, const float w)
		: mData{vec.getX(), vec.getY(), vec.getZ(), w}
	{
	}
	...
	*/	
	...
	const float * Vec4::getDataPtr() const
	{
		return mData;
	}
	...
	float Vec4::getMagnitud()
	{
		if (mMagnitud == -1.0f)
		{
			mMagnitud = sqrtf(mData[0] * mData[0]
				+ mData[1] * mData[1]
				+ mData[2] * mData[2]
				+ mData[3] * mData[3]);
		}
	...
	{
		return left.mData[0] *right.mData[0]
			+ left.mData[1] *right.mData[1]
			+ left.mData[2] *right.mData[2]
			+ left.mData[3] *right.mData[3];
	}
}