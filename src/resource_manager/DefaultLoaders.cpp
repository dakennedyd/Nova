...
namespace Nova {
	std::shared_ptr<ResourceBase> loadTexture(const XMLNode & metadata)
	{
		...
		if (type == "2d")
		{
			...
			if (filtering == "linear") texPtr->setFiltering(Filtering::LINEAR);
			else if (filtering == "nearest") texPtr->setFiltering(Filtering::NEAREST);
			else {
	...
	std::shared_ptr<ResourceBase> loadMaterial(const XMLNode & metadata)
	{
		...
		auto specularColor{ vectorStringToFloat(split(propertiesNode.getAttributeValue("specularColor"))) };
		auto specularIntensity = std::stof(propertiesNode.getAttributeValue("specularIntensity"));
		MaterialProperties properties{ Vec3{ambient[0],ambient[1],ambient[2] }, Vec3{ diffuse[0],diffuse[1],diffuse[2] },
		Vec4{ specularColor[0],specularColor[1] ,specularColor[2], specularIntensity } };*/

		/*MaterialProperties yellowPlastic(Vec3(0.0f), Vec3(0.5f, 0.5f, 0.0f), Vec4(0.6f, 0.6f, 0.5f, 0.25f));
		MaterialProperties jade(Vec3(0.135f, 0.2225f, 0.1575f), Vec3(0.54f, 0.89f, 0.63f), Vec4(0.316228f, 0.316228f, 0.316228f, 0.1f));
		MaterialProperties copper(Vec3(0.19125f, 0.0735f, 0.0225f), Vec3(0.7038f, 0.27048f, 0.0828f), Vec4(0.256777f, 0.137622f, 0.086014f, 0.1f));
		MaterialProperties pearl(Vec3(0.25f, 0.20725f, 0.20725f), Vec3(1.0f, 0.829f, 0.829f), Vec4(0.296648f, 0.296648f, 0.296648f, 0.088f));
		...
		while (!texture.isEmpty())
		{
			...
			/*if (type == "diffuse")
			{
				...
				return std::make_shared<Material>(rm.get<GPUProgram>(program), rm.get<ITexture>(textureName), properties);
			}
	...
	std::shared_ptr<ResourceBase> loadMesh(const XMLNode & metadata)
	{
		...
		if (type == "icosphere")
		{
			...
			return std::make_shared<Mesh>(Mesh::makeIcoSphere(std::stof(radius), std::stoi(iterations)));
		}
		else if (type == "box")
		{
			...
			return std::make_shared<Mesh>(Mesh::makeBox(std::stof(length), std::stof(width), std::stof(height), vectorStringToFloat(split(texCoords))));
		}
		else if (type == "quad")
		{
			std::string width{ param.getAttributeValue("width") };
			std::string height{ param.getAttributeValue("height") };
			std::string texCoords{ param.getAttributeValue("texcoords") };
			return std::make_shared<Mesh>(Mesh::makeQuad(std::stof(width), std::stof(height), vectorStringToFloat(split(texCoords))));
		}
		else if (type == "skybox")
		{
