...
// 				if (!isEqual(c.getX(), glmC[0])) { error("Vec3 add function", c.getX(), glmC[0]); break;}
// 				if (!isEqual(c.getY(), glmC[1])) { error("Vec3 add function", c.getY(), glmC[1]); break;}
// 				if (!isEqual(c.getZ(), glmC[2])) { error("Vec3 add function", c.getZ(), glmC[2]); break;}
// 			}
// 			errorCount = 0;
...
// 				Vec3 c = a - b;
// 				glm::vec3 glmC = glmA - glmB;
// 				if (!isEqual(c.getX(), glmC[0])) { error("Vec3 subtract function", c.getX(), glmC[0]); break;}
// 				if (!isEqual(c.getY(), glmC[1])) { error("Vec3 subtract function", c.getY(), glmC[1]); break;}
// 				if (!isEqual(c.getZ(), glmC[2])) { error("Vec3 subtract function", c.getZ(), glmC[2]); break;}
// 			}
// 			errorCount = 0;
...
// 				Vec3 a(glmA.x, glmA.y, glmA.z);
// 				Vec3 b(glmB.x, glmB.y, glmB.z);				
// 				if (!isEqual(a * b, glm::dot(glmA, glmB))) { error("Vec3 dot product function", a * b, glm::dot(glmA, glmB)); break; }
// 			}
// 			errorCount = 0;
...
// 				if (!isEqual(c.getX(), glmC[0])) { error("Vec3 cross product function", c.getX(), glmC[0]); break; }
// 				if (!isEqual(c.getY(), glmC[1])) { error("Vec3 cross product function", c.getY(), glmC[1]); break; }
// 				if (!isEqual(c.getZ(), glmC[2])) { error("Vec3 cross product function", c.getZ(), glmC[2]); break; }
// 			}
// 			errorCount = 0;
...
// 				if (!isEqual(c.getX(), glmVec4a[0])) { error("Vec3 rotate function", c.getX(), glmVec4a[0]); break; }
// 				if (!isEqual(c.getY(), glmVec4a[1])) { error("Vec3 rotate function", c.getY(), glmVec4a[1]); break; }
// 				if (!isEqual(c.getZ(), glmVec4a[2])) { error("Vec3 rotate function", c.getZ(), glmVec4a[2]); break; }
// 			}
// 			errorCount = 0;
...
// 				//std::cout.precision(std::numeric_limits< float >::max_digits10);
// 				if (!isEqual(a2.getX(), glmVec4a2[0])) { error("Vec3 RotateSelf function", a2.getX(), glmVec4a2[0]); break; }
// 				if (!isEqual(a2.getY(), glmVec4a2[1])) { error("Vec3 RotateSelf function", a2.getY(), glmVec4a2[1]); break; }
// 				if (!isEqual(a2.getZ(), glmVec4a2[2])) { error("Vec3 RotateSelf function", a2.getZ(), glmVec4a2[2]); break; }
// 			}																					  
// 		}
...
// 				for (int i = 0; i < 9; ++i)
// 				{
// 					if (!isEqual(novaMatPtr[i], glmMatPtr[i])) { error("Mat3 inverse function", novaMatPtr[i], glmMatPtr[i]); goto mat3rot; }
// 				}
// 			}
// 		mat3rot:;
// 			errorCount = 0;
...
// 				for (int i = 0; i < 9; ++i)
// 				{
// 					if (!isEqual(novaMatPtr[i], glmMatPtr[i])) { error("Mat3 rotation function", novaMatPtr[i], glmMatPtr[i]); goto mat3end; }
// 				}
// 			}
// 		mat3end:;
// 		}
...
// 				for (int i = 0; i < 16; ++i)
// 				{
// 					if (novaMatPtr[i] != glmMatPtr[i]) { error("Mat4 identity constructor", novaMatPtr[i], glmMatPtr[i]); goto mat4pers; }
// 				}
// 			}
// 		mat4pers:;
// 			errorCount = 0;
...
// 				float aspectRatio = random.nextFloat(0.000001f, 100000.0f) / random.nextFloat(0.000001f, 100000.0f);
...
// 				Mat4 novaMat = Mat4::makePerspectiveMatrix(toRadians(fov), aspectRatio, nearPlane, farPlane);
// 				const float *novaMatPtr = novaMat.getDataPtr();
// 				for (int i = 0; i < 16; ++i)
// 				{					
// 					if (!isEqual(novaMatPtr[i], glmMatPtr[i])) { error("Mat4 perspective function", novaMatPtr[i], glmMatPtr[i]); goto mat4rot; }
// 				}
// 			}
// 		mat4rot:;
// 			errorCount = 0;
...
// 				for (int i = 0; i < 16; ++i)
// 				{
// 					if (!isEqual(novaMatPtr[i], glmMatPtr[i])) { error("Mat4 rotation function", novaMatPtr[i], glmMatPtr[i]); goto mat4mult; }
// 				}
// 			}
// 		mat4mult:;
// 			errorCount = 0;
...
// 			for (int iter = 0; iter < iterations; ++iter)
// 			{
// 				Mat4 a(std::array<float, 16>{random.nextFloat(), random.nextFloat(), random.nextFloat(), random.nextFloat(),
// 					random.nextFloat(), random.nextFloat(), random.nextFloat(), random.nextFloat(),
// 					random.nextFloat(), random.nextFloat(), random.nextFloat(), random.nextFloat(),
// 					random.nextFloat(), random.nextFloat(), random.nextFloat(), random.nextFloat()});
// 				Mat4 b(std::array<float, 16>{random.nextFloat(), random.nextFloat(), random.nextFloat(), random.nextFloat(),
// 					random.nextFloat(), random.nextFloat(), random.nextFloat(), random.nextFloat(),
// 					random.nextFloat(), random.nextFloat(), random.nextFloat(), random.nextFloat(),
// 					random.nextFloat(), random.nextFloat(), random.nextFloat(), random.nextFloat()});
...
// 				for (int i = 0; i < 16; ++i)
// 				{
// 					if (!isEqual(novaMatPtr[i], glmMatPtr[i])) { error("Mat4 multiplication function", novaMatPtr[i], glmMatPtr[i]); goto mat4lookat; }
// 				}
// 			}
// 		mat4lookat:;
// 			errorCount = 0;
...
// 				for (int i = 0; i < 16; ++i)
// 				{
// 					if (!isEqual(novaMatPtr[i], glmMatPtr[i])) { error("Mat4 lookAt function", novaMatPtr[i], glmMatPtr[i]); goto mat4end; }
// 				}
// 			}
// 		mat4end:;
// 		}
...
// 			for (int iter = 0; iter < iterations; ++iter)
// 			{
// 				float a = random.nextFloat(-10000.0f, 10000.0f), b = random.nextFloat(-10000.0f, 10000.0f), c = random.nextFloat(-10000.0f, 10000.0f);
...
// 				LOG_DEBUG(" ");
// 				//if (!isEqual(novaEuler.getX(), glmEuler.x)) { error("Quaternion from Euler angles constructor", novaEuler.getX(), glmEuler.x); break; }
// 				//if (!isEqual(novaEuler.getY(), glmEuler.y)) { error("Quaternion from Euler angles constructor", novaEuler.getY(), glmEuler.y); break; }
// 				//if (!isEqual(novaEuler.getZ(), glmEuler.z)) { error("Quaternion from Euler angles constructor", novaEuler.getZ(), glmEuler.z); break; }
// 				//if (!isEqual(novaEuler.getW(), glmEuler.w)) { error("Quaternion from Euler angles constructor", novaEuler.getW(), glmEuler.w); break; }
// 			}
// 			errorCount = 0;
...
// 				if (!isEqual(c.getX(), quat3.x)) { error("Quaternion multiplication function", c.getX(), quat3.x); break; }
// 				if (!isEqual(c.getY(), quat3.y)) { error("Quaternion multiplication function", c.getY(), quat3.y); break; }
// 				if (!isEqual(c.getZ(), quat3.z)) { error("Quaternion multiplication function", c.getZ(), quat3.z); break; }
// 				if (!isEqual(c.getW(), quat3.w)) { error("Quaternion multiplication function", c.getW(), quat3.w); break; }
// 			}
// 			// errorCount = 0;
...
// 			// 	float d = random.nextFloat(), e = random.nextFloat(), f = random.nextFloat();
// 			// 	Vec3 v(a,b,c);
...
// 				for (int i = 0; i < 16; ++i)
// 				{
// 					if (!isEqual(rotMat.getDataPtr()[i], glmMatPtr[i])) { error("Quaternion toRotationMatrix4 function", rotMat.getDataPtr()[i], glmMatPtr[i]); goto quatend; }
// 					//LOG_DEBUG(MatFromQuat.getDataPtr()[i] << "  " << rotMat.getDataPtr()[i] << "  " << glmMatPtr[i]);
// 				}
// 			}
// 		quatend:;
// 		}
...
// 			}
// 		}
// 		void error(const std::string & failedFunctionName, const float calculatedValue, const float correctValue)
// 		{
// 			if (errorCount < 1) { //only log the first error
// 				LOG_DEBUG("TEST FAILED! in " << failedFunctionName);
// 				LOG_DEBUG("got " << calculatedValue << " should be " << correctValue);
// 			}
// 			errorCount++;
// 			errorCountTotal++;
// 		}
