...
namespace Nova {
	...
	{		
		...
		//LOG_INFO("Load successful. width:" << width << " height:" << height << " bpp:" << bytesPerPixel * 8);		
		TextureInfo t;
		//t.textureBuffer = buffer;
		//t.textureBufferFP = bufferFP;
		if (isHDR) t.textureBuffer.push_back(bufferFP); else t.textureBuffer.push_back(buffer);
		t.width = width;
		t.height = height;
		t.bpp = bytesPerPixel;
		t.isHDR = isHDR;
	...
	const TextureInfo FileSystem::loadTexture(const std::vector<std::string> filesAndPath, const bool uvFlipped)
	{
		stbi_set_flip_vertically_on_load(uvFlipped);
		TextureInfo t;
		int width, height, bytesPerPixel;
	...
	void FileSystem::freeTexture(void * ti)
	{
		stbi_image_free(ti);
	}
	...
	const aiScene * FileSystem::loadModel(const std::string & pathAndFileName)
	{
		...
		const aiScene* scene = import.ReadFile(pathAndFileName, aiProcess_Triangulate | aiProcess_FlipUVs);
	...
	//std::vector<std::string> FileSystem::getFilenamesInDirectory(const std::string & path, bool recursive)
	//{
	...
	//	//if (mypath[mypath.length()-1] != '/' && mypath[mypath.length()-1] != '\\') mypath.push_back('/');
	//	//if ((dir = FindFirstFile((mypath + "*").c_str(), &file_data)) == INVALID_HANDLE_VALUE)
	//	//	return std::vector<std::string>(); /* No files found */
	...
	//	//		if (recursive)
	//	//		{
	//	//			std::vector<std::string> temp = getFilenamesInDirectory(full_file_name, true);
